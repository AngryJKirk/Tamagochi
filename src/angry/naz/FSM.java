    ...
    private void happy() throws InterruptedException, IOException {
        ...
        while (true) {
          ...
          //  telegramBot.notify("I AM HAPPY!");
            creature.changeDream(false);
            creature.changeHungry(false);
            if (creature.getHungry() < 50 & creature.getDream() < 50) {
                state = States.WANTBOTH;
                break;
            } else if (creature.getHungry() < 50) {
                state = States.WANTEAT;
                break;
            } else if (creature.getDream() < 50) {
                state = States.WANTSLEEP;
                break;
            }
